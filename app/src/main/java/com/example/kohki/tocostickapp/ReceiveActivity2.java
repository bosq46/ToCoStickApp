...
/**
 * Created by Kohki on 2016/10/28.
 */

public class ReceiveActivity2 extends Activity{
...
*/
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        fileHandler_ = new FileHandler(this.getApplicationContext(), "sensor_data.txt");
...
//                byte[] bytes_message = new byte[len];
                    ...
                    mHandler.post(new Runnable() {
                        ...
                        public void run() {
                            ...
                            for (int i=0; i<rbuf.length; i++) {
                                ...
                                sb_hexbuf.append(Integer.toHexString((rbuf[i] & 0xF0) >> 4));
                                sb_hexbuf.append(Integer.toHexString(rbuf[i] & 0xF));
                            }
                            writeData(sb_hexbuf);
    ...
    private void writeData(CharSequence message) {
        ...
        try {
            fileHandler_.writeStrFile(sdf.format(date) +" "+ message);//savefile()内は、書き込みの最後に改行を自動で挿入される
            readmessages = fileHandler_.readStrFile();
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private void updateFileLinesView(){
        ...
        try {
            filelines = fileHandler_.readStrFile();
